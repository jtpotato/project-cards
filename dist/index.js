...
// src/template.txt
var template_default = `<svg xmlns="http://www.w3.org/2000/svg" width="640" height="280">
    <defs>
...
var handler = {
  async fetch(request) {
    ...
    let age = import_dayjs.default(started).fromNow();
    let text = template_default;
    let newSvg = text.replace("{{start_color}}", leftCol).replace("{{stop_color}}", rightCol).replace("{{codename}}", codename ?? "Unknown").replace("{{project_name}}", project ?? "Unknown").replace("{{start_date}}", started ?? "Unknown").replace("{{age}}", age.toString() ?? "Unknown");
